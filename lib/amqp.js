...
module.exports = function (config, logger) {
  ...
  var channel = _.partialRight(createChannel, config.connection.useConfirms);
 ...
 * @return {AmqpChannel}
 */
function createChannel(connection, useConfirms) {
  return useConfirms ? connection.createConfirmChannel() : connection.createChannel();
}
...
function defineUrl(config) {
  ...
  var credentials = [config.user, config.pass].join(':');
  var hostPort = [config.host, config.port].join(':');

  return 'amqp://' + credentials + '@' + hostPort + '/' + encodeURIComponent(config.vhost);
}
